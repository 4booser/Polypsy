...
const tabClass = (isActive: boolean) =>
  cx(
     ...
     */
    /*
      Бледная — АКТИВНАЯ, полная — остальные. Так на кадрах: замер
      f05 и f06 по преобладающему цвету чернил вкладки («Моя група»
      на обоих кадрах активна по index.json) даёт (178,153,204) у
      активной и (102,51,153) у всех прочих. Прежде ветки стояли
      наоборот — по привычке «выбранное темнее», а не по кадру.

      Кадровый бледный (#b299cc) на белом — 2,52:1, ниже нормы
      вдвое, поэтому вместо него стоит --primary-dim (6,06:1): та же
      мысль «вполсилы» читаемой. Различие между вкладками осталось
      по светлоте, а не по тону, — переживает дальтонизм и ч/б, — а
      «вы здесь» несут aria-selected и aria-current, которые от
      цвета не зависят вовсе.
    */
    isActive ? "text-primary-dim" : "text-primary",
  );
...
export function Tabs({ items, label }: { items: TabItem[]; label?: string }) {
  const bar = cx(
     ...
     * вкладок три.
     *
     * Замер f05/f06: полоса 7px (y 255…261), дорожка БЕЛАЯ с кромками в
     * пиксель #cccccc сверху и снизу, ползунок сплошной #999999. Прежние
     * 4px со сплошной серой дорожкой рисовали вдвое тоньше и без кромок:
     * на кадре внутри дорожки виден лист, а не серое.
     *
    ...
    "[&::-webkit-scrollbar-track]:shadow-[inset_0_1px_0_var(--hairline),inset_0_-1px_0_var(--hairline)]",
    ...
    <nav aria-label={label} className={bar}>
      {items.map((t) => (
        <NavLink
           ...
           * только цвет: бледный #b299cc у активной против полного #663399 у
           * остальных (замер f05/f06), начертание и кегль у обеих одинаковые
           * (18/700). Цвет как единственный признак — нарушение WCAG 1.4.1.
           *
