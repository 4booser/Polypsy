 ...
 *
 * 1. Кеглей ДВА, а не один. Капитель Onest — 0,700 кегля (перепроверено на
 *    «У» слова «Укр»: 20/700 даёт ровно 14 px). Крупные буквы: 41 / 1,9429 /
 *    0,70 = 30,1 → 30. Мелкие: 25 / 1,9429 / 0,70 = 18,4 → 18,5. Общий
 *    `fontSize="16"` давал всем пяти капитель 21,8 px вместо 41 и 25 — знак
 *    читался ровным «POLSY» вместо композиции макета.
 ...
 *
 * Кольцо: срез по y = 241 даёт сплошные три пикселя — 3 / 1,9429 = 1,55
 * единицы viewBox (подвальный знак 80 подтверждает: там та же дуга ≈ 1,7 px
 * при делителе 1,1429). Стояло 2, то есть 3,89 px на 136, — кольцо было
 * толще макетного на четверть.
 *
...
export function Logo({ size = 70 }: { size?: number }) {
  return (
    ...
    >
      <circle cx="35" cy="35" r="34" fill="none" stroke="currentColor" strokeWidth="1.55" />
  ...
  return (
    <button
      ...
      className={cx(
         ...
         * норме раздела 44 (WCAG 2.5.5). Площадка растянута до 44 × 44, а
         * отрицательные поля по вертикали возвращают строке её 20 px в
         * потоке: от низа строки «Укр» отсчитан отступ 97 до знака, и без
         * втягивания знак съехал бы с замеренных 174 на кадре f00. Приём тот
         * же, что у круглых значков соцсетей в подвале (PublicFrame.tsx), и
         * та же цена: соседние площадки перекрывались бы — но слева и справа
         * от «Укр» в шапке пусто, перекрывать нечего.
         *
         * Влево площадка не втянута: левая кромка чернил стоит на 317 — это
         * начало колонки плюс отступ 117 с кадра, и сдвигать её нельзя.
         * Лишние 11 px вправо приходятся на пустое поле.
         */
        "-my-[12px] flex h-[44px] min-w-[44px] items-center",
