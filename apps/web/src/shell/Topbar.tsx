...
/*
 * ПОЛОСА БЫВАЕТ ЧЕТЫРЁХ СОСТАВОВ, А НЕ ДВУХ.
 *
 * Кадры раздела людей рисуют четыре разных верхних меню, и это не редакции
 * одного, а четыре рабочих места — по ступени лестницы должностей
 * (specialist → head → chief → суперадмин, см. ROLE_LADDER):
 *
 ...
 *                  лікарі;
 *   peopleStaff  — ОДИН пункт «Лікарі» по центру (f40, f41, f42, f43):
 *                  консоль людей администратора. Он никого не ведёт, у него
 *                  нет ни своих пациентов, ни своих методик, и пять
 *                  клинических разделов в его полосе вели бы в пустоту;
 *   peopleAdmins — ОДИН пункт «Адміністратори» по центру (f47, f48, f49,
 *                  f50): консоль суперадмина. «Адміністраторів» в полосе не
 *                  было ни в каком виде — пункт жил только в бургере
 *                  (Rail.tsx), и суперадмин видел чужую полосу.
 *
 * Пункты, ушедшие из полосы в двух последних составах, НЕ пропали: весь
 * набор разделов целиком лежит в бургере (railGroups), и он от состава
 * полосы не зависит. Полоса — ярлыки рабочего места, бургер — все двери.
 */
...
/**
 * Какое рабочее место у вошедшего.
 *
 * Отдано наружу и считается по одним и тем же двум полям (класс записи и
 * ступень лестницы), потому что тот же вопрос задаёт карточка человека: под
 * карточкой в общей консоли лежит список или плитки (f04, f30, f31), а в
 * консоли людей — ничего, кроме подчинённых лікарів у чужой карточки
 * администратора (f40, f43, f47 против f50). Два ответа на один вопрос
 * разошлись бы на первой же правке.
 */
export function barKind(user: { role?: string | null; ladderRank?: number | null }): BarKind {
  if (user.role === "superadmin") return "peopleAdmins";
  const rank = user.ladderRank ?? 0;
  /* chief (3) и выше — чистый администратор: лестница кончилась, лечить некого */
  if (rank >= 3) return "peopleStaff";
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cx(
                  ...
                  */
                  /*
                    Кадры спорят о том, есть ли подсветка вообще: f42 рисует
                    «Лікарі» полным #663399 на том же экране списка, где f49
                    рисует «Адміністратори» бледными. Взят f49 — с ним
                    согласны все четыре остальных кадра с совпадающим адресом
                    (f30, f31, f34, f35), а f42 остаётся единственным.
                  */
