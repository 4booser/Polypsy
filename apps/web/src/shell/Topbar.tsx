...
function MoreMenu({
   ...
   *
   * Запоминается адрес окна, а не маршрутизатора. Переход маршрутизатор
   * проводит переходом React (startTransition): адрес в строке уже новый, а
   * отрисован ещё прежний экран, пока догружается кусок нового. Сразу после
   * входа это длится заметно — оболочка уже стоит, а `pathname` всё ещё
   * `/login`. Меню, открытое в этот миг, принимало догнавший маршрутизатор
   * за переход и закрывалось через десятки миллисекунд после открытия.
   * Сверка с окном отличает переход от «маршрутизатор догнал адрес».
   */
  const seen = useRef(window.location.pathname);
  useEffect(() => {
    const now = window.location.pathname;
    if (seen.current === now) return;
    seen.current = now;
    onClose();
  }, [pathname, onClose]);
