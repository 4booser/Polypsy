...
/*
 * Карточка пациента — кадр f13 макета, один свиток сверху вниз:
 *
 ...
 *                                        графики или протокол — см. ниже),
 *                                        «Результат тесту», «Статистика»
 ...
 * - Имя строки теста ведёт на «Графіки» прохождения
 *   (/surveys/:id/responses/:rid/charts), а не на протокол, если у
 *   прохождения есть баллы шкал; без баллов (подсчёт выключен) — на протокол
 *   «Відповіді», как раньше: графикам без баллов показать почти нечего.
 *   Решение заказчика 2026-09-26: «нажать на прохождение теста от
 *   определённого пациента — и показало визуально графики». Остальные входы
 *   в прохождение (аналитика методики, разбор случаев) ведут на протокол по-
 *   прежнему: там ищут ответ на конкретный пункт, а не картину целиком.
 ...
 * - Подсвеченная первая строка «Групи» на кадре f13_2 прочитана как наведение,
          ...
          <ul className="m-0 list-none p-0">
            {card.responses.map((r) => (
              <Row
                key={r.responseId}
                name={r.surveyTitle}
                /* с баллами — сразу на графики (решение заказчика 2026-09-26, см. шапку файла) */
                to={
                  r.scales.length
                    ? `/surveys/${r.surveyId}/responses/${r.responseId}/charts`
                    : `/surveys/${r.surveyId}/responses/${r.responseId}`
                }
              >
 ...
 * оси: кадры о смещении «+» спорят (f13_1 даёт «+» на 4 ниже прописных,
 * f13_2 — на 3 выше), и середина спора — это и есть одна ось.
 *
 ...
 * f13_2 так подсвечена первая строка «Групи»): `has-[a:hover]` красит строку,
  ...
  return (
    ...
    >
      {/*
        ...
        сжаться блоку, но не переносит слово. Перенос — по краю колонки; наезд
        на «Результат тесту» хуже любого переноса.

        Не по слогам (hyphens: auto), хотя так красивее: слоговой перенос
        зависит от словаря, который у браузера то есть, то нет. На раннере CI
        «эмо-ционального» переносилось по слогам, в контейнере эталонов — нет,
        и снимок карточки краснел на ровном месте; у людей в разных браузерах
        строка ломалась бы так же по-разному.
      */}
      <Link
        to={to}
        className={cx(
 ...
 *
 * Кадр f13 рисует два вида: светлый (заливка #f0ecff, --primary-soft) —
 * название теста, и залитый сиреневым (#b299cc, --primary-rule) — название
 * аналитики. Здесь написан только светлый, и это не недосмотр.
 *
 * Аналитики как ИМЕНОВАННОГО объекта, который можно положить в заключение,
 * на сервере нет: в ответе карточки у заключения одно поле surveyTitle
 * (см. отчёт, строка «сервер»). Второй вид был написан «на будущее» и
 * остался веткой, которую никто не вызывает: она не падает, не видна и
 * потому не чинится — и первый же читатель принимает её за работающую
 * возможность. Пока сервер не отдаёт имён, вида нет вовсе; расхождение с
 * кадром записано в отчёт словами, а не мёртвым кодом.
 */
