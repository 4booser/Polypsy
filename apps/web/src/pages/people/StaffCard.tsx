 ...
 *   только потом строка раздела.
 *
 *   Подменю рисует ОДИН кадр из двух: на f34 под линией сразу идёт строка
 *   «Групи» (поле 254…289), на f35 между ними стоят семь пунктов (чернила
 *   229…250). Взят f35 — он полнее, и без подменю раздел карточки не
 *   сообщал бы, где человек находится и куда из этого раздела можно. По
 *   имени кадры расходятся так же: на f35 оно по центру (чернила 613…996,
 *   середина 804 при середине колонки 806), на f34 сдвинуто вправо
 *   (661…1044, середина 852 при 810). Взят опять f35 — шапка раздела
 *   собрана с одного кадра, а не сшита из двух.
 *
 ...
 *
 * ЗАГОЛОВОК ПРОФИЛЯ — не «своя или чужая», а «в каком разделе» (подробно у
 * самого заголовка, ниже):
 *
 *   общая консоль: своя — СЛОВО РОЛИ «Лікар» (f04, f30), чужая — ПІБ (f31,
 *   f34, f35). Только имя отличает карточку одного человека от карточки
 *   другого в истории браузера, в закладке и в заголовке окна;
 *
 *   раздел людей: ВСЕГДА слово роли просматриваемого — «Адміністратор»
 *   (f40 своя, f50 чужая), «Лікар» (f43 чужая), «Супер Адміністратор» (f47
 *   своя). Здесь имя не теряется: оно стоит первыми двумя полями карточки.
 *
 ...
 *
 * ГДЕ ДВЕРЬ В РАЗДЕЛЫ КАРТОЧКИ, раз вкладок больше нет:
 *
 *   чужая карточка в общей консоли — шесть плиток (f31);
 *   своя карточка лікаря           — заголовок секции «Пацієнти» (ссылка на
 *                                    /staff/:id/patients), а оттуда подменю
 *                                    f35 ведёт в «Групи» и дальше;
 *   любой раздел карточки          — подменю (f35).
 *
 * У своей карточки заведующего (f30) под разделителем стоит справочник
 * лікарів, а не его собственный список, — заголовок ведёт туда же, на
 * /staff. Его собственные «Пацієнти» и «Групи» при этом никуда не делись: у
 * своей карточки разделы карточки — те же экраны, что «Пацієнти» и «Групи» в
 * полосе, и полоса у него из семи пунктов.
 *
 * В разделе людей (f40, f43, f47, f50) кадры под карточкой не рисуют ни
 * плиток, ни списка, и отдельной двери в разделы карточки там нет: у
 * администратора нет ни своих пациентов, ни своих групп, а чужие открываются
 * из общей консоли. Записано здесь, а не оставлено молча.
 */
...
export default function StaffCard() {
  ...
  /* тот же ответ, что у полосы: состав считается по разделу и по праву видеть пациентов */
  const kind = barKind(user, user?.role !== "admin" || can("patients.read"), pathname);
  ...
  /*
   * ЗАГОЛОВОК КАРТОЧКИ. Правило, которое сводит все восемь кадров карточки
   * без исключения, — не «своя или чужая», а «в каком разделе»:
   *
   *   общая консоль  своя  → «Лікар»            (f04 у специалиста, f30 у
   *                                              заведующего: он лечит, и под
   *                                              его карточкой список лікарів)
   *                  чужая → ПІБ                (f31, f34, f35)
   *   раздел людей   любая → слово роли ТОГО, ЧЬЯ КАРТОЧКА:
   *                          «Адміністратор» своя у администратора (f40),
   *                          «Лікар» чужая у него же (f43),
   *                          «Супер Адміністратор» своя у суперадміна (f47),
   *                          «Адміністратор» чужая у него же (f50).
   *
   * Здесь стояло «чужая — всегда ПІБ», и f43/f50 оставались не сведены: имя
   * печаталось там, где кадр печатает роль. Довод «только имя отличает одну
   * карточку от другой» остаётся верным для общей консоли — в разделе людей
   * его перевешивает кадр, а имя никуда не девается: оно первыми же двумя
   * полями стоит на самой карточке.
   *
   * Слово роли — по ПРОСМАТРИВАЕМОМУ, а не по смотрящему: иначе суперадмін
   * на f50 подписал бы чужую карточку своим «Супер Адміністратор». Кто
   * просматриваемый, говорит справочник, из которого карточка открыта:
   * суперадмін ведёт администраторов (f49 → f50), администратор — лікарів
   * (f42 → f43). Ступень лестницы самого просматриваемого для этого не
   * годится: в карточку она приезжает отдельным запросом (StaffProfile,
   * perms), и заголовок <h1> менялся бы на глазах через полсекунды после
   * открытия. Цена решения названа вслух: суперадмін, открывший карточку
   * лікаря прямой ссылкой, увидит над ней «Адміністратор» — кадра на этот
   * случай нет, а имя стоит первыми двумя полями карточки.
   *
   * «Супер Адміністратор» — два слова с заглавной в обоих, именно так на
   * кадре, в отличие от слитного nav.roleSuper, которым назван класс записи
   * в поле «Роль».
   *
   * Само правило — cardTitleRole в model.ts: «имя или роль» проверяется там
   * без React, потому что глазами такая подмена ловится только случайно.
   */
  const roleKey = cardTitleRole(kind, me);
  ...
  if (!onProfile) {
    return (
      <Page
        ...
        titleAlign="center"
        /* 40 от низа полосы: линия под именем стоит на f34/f35 в 213…214, то есть в 100 от низа полосы */
        topGap={40}
      >
  ...
  return (
    <Page
      title={roleKey ? ut(roleKey) : name}
      /* 40 от низа полосы: чернила заголовка на всех семи кадрах профиля стоят в 46-48 */
      topGap={40}
      actions={
        <ActionMenu
 ...
 *
 * Цена названа вслух: тот, кто видит экран, но не слышит диктора, признака
 * «вы здесь» в подменю не получает — он есть только в адресной строке и в
 * строке раздела под подменю, где стоит заголовок «Групи». Добавить сюда
 * цвет или черту значило бы нарисовать то, чего на кадре нет; заголовок
 * раздела эту же работу делает и нарисован. Общие вкладки (Tabs) при этом не трогаются:
        ...
        <>
          ...
          */}
          ...
          {under === "tiles" ? (
            ...
            <OwnPatients heading to={`/staff/${row.id}/patients`} />
          ) : (
            <StaffSection me={me} kind={kind} />
          )}
        </>
      )}
    </>
...
function StaffTiles() {
  ...
  return (
    <nav
      aria-label={ut("ppl.tilesLabel")}
      /*
       * mt-6 поверх штатных 29 от разделителя: на f31 линия лежит на
       * 432-433, а верх первого ряда плиток — на 469, то есть 35 от низа
       * линии против 29 у заголовка секции на f04/f30. Плитки стоят ниже
       * заголовка, и это замер, а не округление.
       */
      className="mt-[6px] grid grid-cols-3 gap-x-[16px] gap-y-[15px] max-[900px]:grid-cols-1"
    >
 ...
 * словом роли.
 */
function SectionBar({ title, to, children, actions }: { title: string; to?: string; children?: ReactNode; actions?: ReactNode }) {
  const headingClass = "m-0 shrink-0 text-[24px] font-bold leading-tight text-primary";
  return (
    /*
     * Подвал строки — 28, когда в ней стоит поле (f50: коробка 459…494,
     * имя первой строки 534 при расчётных 537), и 11, когда в строке один
     * заголовок (f04: чернила «Пацієнти» 467…487, имя 514; f30: 470…491 и
     * 517). Разница не в отступе, а в высоте самой строки: поле 36 против
     * 30 у заголовка, и прежние общие 28 роняли список на 17px ниже кадра
     * там, где поля нет.
     */
    <div className={cx("flex items-center gap-[24px] max-[900px]:flex-wrap", children || actions ? "mb-[28px]" : "mb-[11px]")}>
      {/*
        Заголовок секции — ссылка туда, где этот список живёт своим
        разделом, и ровно тот же на вид: тот же кегль, тот же цвет, без
        подчёркивания. Это и есть дверь в разделы карточки со своего
        профиля: вкладок «Профіль · Пацієнти · Групи» на кадрах нет, плитки
        рисует только чужая карточка (f31), а подменю живёт на самих
        разделах (f35) — без этой ссылки адрес /staff/:id/patients остался
        бы без входа. Куда ведёт — решает вызывающий: под своей карточкой
        лікаря это раздел карточки, под карточкой заведующего — общий
        справочник, который в той секции и показан.
      */}
      {/* ссылка внутри заголовка, а не вместо него: порядок h1 экрана → h2 секции сохраняется */}
      <h2 className={headingClass}>
        {to ? (
          <Link to={to} className="text-primary no-underline outline-none hover:underline focus-visible:ring-2 focus-visible:ring-[var(--focus)]">
            {title}
          </Link>
        ) : (
          title
      ...
      {/*
        Между «+» и блоком страниц — 87, а не общие 24 строки: замер f34
        («+» 1083…1109, то есть ровно 27 кнопки, страницы с 1197 и до края
        колонки 1409). 24 штатных плюс 63 здесь. Без этого поле, растянутое
        на весь остаток, уводило «+» на полсотни вправо: страницы прижаты к
        правому краю колонки и с места не сходят, а «+» плывёт за полем.
        С этим числом поле выходит 768 при кадровых 767.
      */}
      {actions ? <div className="ml-[63px] flex shrink-0 items-center gap-[14px]">{actions}</div> : null}
    </div>
  );
}
...
function OwnPatients({ heading, to }: { heading?: boolean; to?: string } = {}) {
  ...
  return (
    <>
      {heading ? <SectionBar title={ut("top.patients")} to={to} /> : null}
      {page.error ? (
...
/**
 * Секция «Лікарі» — два кадра, и они не одинаковы.
 *
 *   f30 (своя карточка заведующего): в строке ТОЛЬКО заголовок. Чернила
 *   «Лікарі» 470…491, и во всей полосе y 435…520 нет ни одной строки шире
 *   50px цвета (102,102,102) — рамки поля там нет вовсе;
 *
 *   f50 (чужая карточка администратора у суперадміна): заголовок и широкое
 *   поле с лупой во всю оставшуюся ширину колонки (коробка 332…1410 при
 *   колонке 214…1412), БЕЗ «+».
 *
 * Поэтому поле здесь — свойство чужой карточки, а не секции: искать в своём
 * же справочнике, который весь перед глазами, кадр не предлагает.
 *
 * На f50 под каждой строкой лежит линия #cccccc — своя сетка, а не общая: на
 * f04/f30/f42/f49 линий нет, и общий gridClass их не получает.
 *
...
function StaffSection({ me, kind }: { me: boolean; kind: BarKind }) {
  ...
  const viewerId = user?.id ?? "";
  /*
   * Линии между строками — только там, где они нарисованы (f50), и
   * ВНУТРЕННЕЙ ТЕНЬЮ, а не рамкой: рамка прибавлялась к высоте строки, и шаг
   * становился 60 при кадровых 58. Замер f50: разделители 2px (204,204,204)
   * на 577-578, 635-636, 693-694 — шаг ровно 58, то есть линия лежит внутри
   * строки, а не под ней.
   */
  ...
  return (
    <>
      {/* заголовок ведёт в общий справочник — тот самый, что в этой секции и показан (f30) */}
      <SectionBar title={ut("ppl.staff")} to={me ? "/staff" : undefined}>
        {me ? null : <SearchBox label={ut("adm.searchPlaceholder")} value={q} onChange={setQ} />}
      </SectionBar>
      ...
      {!me ? (
        ...
        <ul className={cx("m-0 list-none p-0", gridClass)}>
          {shown.map((r) => (
            <li key={r.id} className={cx("min-w-0", ruled && "shadow-[inset_0_-2px_0_var(--hairline)]")}>
