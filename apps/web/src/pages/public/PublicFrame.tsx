 ...
 *
 * Разделы «FAQ» и «Для преси» ведут на якоря лендинга: отдельных кадров у
 * них нет, а страница без кадра — это страница, нарисованная наугад. Когда
 * заказчик даст тексты, у якорей появятся свои блоки — ссылки менять не
 * придётся. Телефон и почта — настоящие ссылки (tel:, mailto:), а не текст:
 * на телефоне по контакту нажимают, а не переписывают.
 *
 * Год в «©» — текущий, а не «2023» с кадра: 2023 — год, когда макет рисовали,
 * а не часть оформления, и застывший год на живом сайте читается как «сайт
 * заброшен». Вернуть буквальное «2023» — одна строка ниже.
 */
function Footer() {
  const { ut } = useLang();
    ...
    <footer className="bg-surface-2">
      ...
      >
        ...
        */}
        ...
        <div className="mt-auto text-center">©{year}</div>
      </div>
    </footer>
  );
}
