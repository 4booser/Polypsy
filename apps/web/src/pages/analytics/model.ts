 ...
 *
 * Чего правило не умеет, а макет рисует (перечислено в отчёте волны):
 *   · «Або» между блоками — условия соединяются только «Разом з» (evaluateRules
 *     требует every(met)); блок здесь один;
 *   · условие по вопросу и ответу и «Час реакції» — таких видов условия нет;
 *   · «Об'єднати» и «Додати Оператор» — при одном операторе объединять и
 *     выбирать нечего.
 * Экран не рисует ни одного из этих элементов: глиф без действия хуже
 * отсутствия глифа — его нажмут.
 */
...
/**
 * Сравнение — словами, а не знаком: на кадре f27 читается «Не менше» рядом с
 * узким числовым полем. Знак «≥» человек в кабинете переводит в голове, слово
 * — нет.
 */
 ...
 * ScaleCondition). В черновике это отдельное значение селекта, а не пустая
 * строка: пустая строка — «тест ещё не выбран», и её сохранять нельзя.
 */
 ...
 * меню шестерёнки до первого поля. На кадре f15 блок тоже нарисован уже с
 * параметром внутри.
 */
