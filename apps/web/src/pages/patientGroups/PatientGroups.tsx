...
/*
 * «Групи» — кадр f06 макета: каталог групп ПАЦИЕНТОВ.
 *
 ...
 * Активная набрана полным фиолетовым (замер f21/f22/f07, где видно, какая
 * вкладка открыта), неактивная — бледной; здесь все шесть бледные, как
 * непройденные двери.
 *
 * Сердце нарисовано и работает, но живёт в браузере, не на сервере — почему,
 * см. FAVORITES_KEY в model.ts.
 *
 * Числа рядом с «Групи» нет: на кадре его нет, каталог тестов (f07) его
...
export default function PatientGroups() {
  ...
  return (
    <Page
      title={ut("pg.title")}
      /* блок страниц отодвинут от «+» на 74, как на кадре f05/f06 */
      pagerGap={74}
      toolbar={
        /* поле тянется от заголовка до «+», лупа — украшение: поиск идёт по мере набора */
        /* зазор «поле → +» 14 — по f05 (поле до 1022, «+» 1036…1062); на f06 он 21,
           и взят f05 по тому же доводу, что и просвет блока страниц: у него два упора, а не один */
    ...
    >
      ...
      */}
      ...
      {list.error ? (
         ...
         */
        /*
         * Зазор 62, колонка 569. Число снято не по чернилам, а по ПОДЛОЖКЕ
         * наведения: на f06 подсвечена одна строка правой колонки, и её
         * прямоугольник — это и есть ячейка сетки, x 831…1399 ровно
         * (подложка --primary-tint, 247,245,250). Отсюда колонка 569, шаг
         * 631 и зазор 1200 − 2×569 = 62. По чернилам столько не снять:
         * текст кончается там, где кончилось слово (на f06 левая колонка
         * доходит до 767), и прежние 30 и 48 взяты как раз так.
         */
        <ul className="m-0 mt-[22px] grid list-none grid-cols-2 gap-x-[62px] p-0 max-[900px]:grid-cols-1">
          {rows.map((g) => (
            <GroupRow
              key={g.id}
              group={g}
