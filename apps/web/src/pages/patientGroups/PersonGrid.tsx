...
/*
 * Список людей в три колонки — кадры f05 и f14 макета.
 *
 ...
 * то есть галочка копит выборку под действие; на f14 нарисована та же
 ...
 * — и раскрывает его сам счётчик (SelectionBar): на кадре под сеткой
 * напечатано только «18 вибрано», и больше в том рядке нет ничего.
 */
...
/**
 * «18 вибрано» — нижний рядок кадра f05, и на кадре в нём БОЛЬШЕ НИЧЕГО нет.
 *
 * Замер всей ширины колонки в этом рядке (f05, y 849…866): чернила только
 * x 201…296, цвет (102,102,102), правее — чистый лист до 1400. Ни кнопок,
 * ни ссылки «Зняти вибір», ни глифа «⋯», — а кадр снят как раз в состоянии
 * с выборкой, то есть молчание тут значащее.
 *
 * Прежняя редакция ставила рядом со счётчиком «⋯». Это и есть расхождение:
 * глиф на кадре не нарисован. Поэтому меню раскрывает САМ СЧЁТЧИК — ближе
 * к выборке места нет, а рядок при этом печатает ровно то, что на кадре.
 * Цена честная: неподсвеченная надпись хуже видна как кнопка, поэтому под
 * курсором и под фокусом она получает подчёркивание — состояний наведения
 * кадр не рисует вовсе, и запрета на них нет.
 *
 * Действия приходят списком, а не разметкой, и меню появляется только с
 * выборкой: пункт «Додати до групи» при нуле выбранных обещал бы действие,
 ...
 * нашёл бы только Tab-ом до конца. `aria-atomic`: меняется одно число, а
 * прочесть надо всю фразу, не «три».
 */
export function SelectionBar({ count, entries }: { count: number; entries?: MenuEntry[] }) {
  const { ut } = useLang();
  const label = (
  ...
  return (
     ...
     */
    <div className="mt-[40px] flex flex-wrap items-center text-[20px] leading-none text-muted">
      {count > 0 && entries?.length ? (
        <ActionMenu
          ...
          triggerSize="sm"
          /*
           * Восклицательный знак — не украшение. Раскрывающая кнопка внутри
           * MenuButton нарисована `variant="ghost"`, а тот несёт `text-primary`
           * и `hover:bg-primary-soft`; спор двух правил равной силы Tailwind
           * решает не порядком в строке классов, а порядком в собранном CSS,
           * и там `.text-primary` стоит ПОСЛЕ `.text-muted` — счётчик вышел бы
           * фиолетовым вместо кадровых (102,102,102). Проверено в сборке.
           */
          triggerClassName="h-auto px-0 text-[20px] font-normal text-muted! hover:bg-transparent! hover:underline"
          entries={entries}
        />
      ) : (
        label
