...
function Row({ m }: { m: MailingListItem }) {
  return (
    ...
    >
      <Link
        to={mailingHref(m.id)}
        className="w-[156px] shrink-0 text-[17px] font-bold leading-[20px] text-primary no-underline hover:underline"
      >
        {m.title}
      </Link>
      ...
      {/*
        ...
        Колонка не `w-[96px]`, а `min-w-[96px]`: day() дописывает год, когда он
        не текущий («14 трав. 2025 20:15»), и в жёсткие 96 это не влезало —
        ...
        то есть у́же 96 и прижата вправо), а длинный вариант раздвигает колонку
        за счёт текста, у которого min-w-0 и обрез по трём строкам.
        Отвергнуто: обрез самой даты (truncate) — «14 трав. 202…» хуже, чем
        строка текста на пару знаков короче.
      */}
      <span className="min-w-[96px] shrink-0 whitespace-nowrap text-right text-[14px] font-bold leading-[23px] text-muted">
