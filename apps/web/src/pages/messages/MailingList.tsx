 ...
 * числом: поедет за палитрой. На кадре полоса тянется до левого края
 * экрана — это артефакт рисунка (справа она кончается на краю колонки);
 * здесь она в границах колонки.
 */
function Row({ m }: { m: MailingListItem }) {
  return (
    <li
      className={cx(
        "flex items-start gap-[24px] py-[13px]",
