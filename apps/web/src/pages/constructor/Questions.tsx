                    ...
                    {q.options.map((o, oi) => (
                      <tr key={oi}>
                        ...
                        <td className="w-[110px]">
                          {/*
                            «yes / no» здесь не переводится, и это не
                            недосмотр: подсказка показывает не слово, а образец
                            значения. Код ключа уходит в scoring.ts и
                            сравнивается с matchKey методики; во всех
                            встроенных методиках (mlo, minimult, pq16, sr45,
                            sadPersons) он записан именно как "yes"/"no".
                            Подсказка «так / ні» научила бы автора набирать
                            кириллицу — и ключ, собранный по образцу, молча
                            перестал бы совпадать: баллы не начислялись бы, а
                            ошибки на экране не было бы. Подпись же колонки
                            («Код ключа») переведена — переведено то, что
                            объясняет, и оставлено то, что показывает.
                          */}
                          <Input
                            value={o.keyCode ?? ""}
                            placeholder="yes / no"
