...
describe("украинский не сползает в русский", () => {
  ...
  test("в украинских строках нет русских букв", () => {
    const wrong: string[] = [];
    for (const [key, v] of Object.entries(UI as Record<string, { uk: string }>)) {
      const hit = [...v.uk].filter((ch) => "ыъэё".includes(ch));
      if (hit.length) wrong.push(`${key}: ${[...new Set(hit)].join("")} — «${v.uk}»`);
    }
    expect(wrong).toEqual([]);
  });

  /*
   * Одно и то же слово — одной формой на весь интерфейс.
   *
   * «Вже» и «уже» оба существуют в украинском, и по отдельности ни одно не
   * ошибка. Но словарь писал то так, то иначе — семь раз «вже», четыре
   * «уже», — и на соседних экранах одна и та же мысль выглядела набранной
   * разными руками. Правило не «эта форма правильная», а «форма одна».
   *
   * Список намеренно короткий: сюда попадает пара, которая уже разъехалась,
   * а не всё, что теоретически может.
   */
  /*
   ...
   */
  const ONE_FORM: [RegExp, string][] = [[/(?<!\p{L})[Уу]же(?!\p{L})/u, "вже"]];

  test("украинский держится одной формы слова", () => {
    const wrong: string[] = [];
    for (const [key, v] of Object.entries(UI as Record<string, { uk: string }>)) {
      for (const [pattern, keep] of ONE_FORM) {
        const m = v.uk.match(pattern);
        if (m) wrong.push(`${key}: «${m[0]}» — в интерфейсе принято «${keep}»`);
      }
    }
    expect(wrong).toEqual([]);
  });
