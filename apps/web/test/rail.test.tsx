 ...
 * Разделы меню.
 *
 * Боковой рельсы на экране больше нет — её место заняла верхняя полоса, а
 * весь набор разделов переехал в бургер справа. Собирает его та же
 * railGroups: имя осталось от прежней раскладки, а список разделов консоли
 * как был единственным, так и остался — его же читает экран настроек
 * («убрать с глаз»). Поэтому проверка не переписана под новую разметку, а
 * осталась там, где была: она про состав навигации, а не про то, колонкой та
 * нарисована или строкой.
 *
 ...
 * заводились отдельной группой с ключом группы пациентов, и в меню
 ...
 *
 * Прятать пункты нужно — меню выросло до тринадцати разделов, и половиной из
 * них конкретный специалист не пользуется. Но три из них носят не раздел,
 ...
 * сломать.
 */
describe("скрытие разделов меню", () => {
  ...
  test("убранный раздел исчезает из меню", () => {
    expect(keysOf([]), "«Методики» нет в меню и без всякого скрытия").toContain("nav.surveys");
    expect(
      keysOf(["nav.surveys"]),
  ...
  test("опустевшая группа исчезает целиком", () => {
     ...
     * Проверяется на «Методиках»: прячутся ВСЕ её пункты, какие бы там ни
     * стояли, — перечень из двух имён ломался бы каждый раз, когда в группу
     * добавляют третий (так и вышло с «Групами методик»).
     */
    const methods = railGroups(counts, true, true).find((g) => g.key === "nav.group.methods");
    expect(methods, "группы «Методики» нет — проверять нечего").toBeDefined();
    const groups = railGroups(counts, true, true, methods!.items.map((i) => i.key));
    expect(
      groups.map((g) => g.key),
      "пустая группа осталась в меню заголовком без содержимого",
