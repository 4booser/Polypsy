...
describe("RLS покрывает все клинические таблицы", () => {
  test("ни одна таблица с клиническими данными не осталась без политик", async () => {
    ...
    const mustSee = [
      ...
      "department_patients",
      /*
       * Состав групп пациентов. Определитель находит его по колонке
       * patient_id — но найдёт он её только пока эта колонка так и зовётся.
       * Названа поимённо, чтобы переименование или сужение списка колонок
