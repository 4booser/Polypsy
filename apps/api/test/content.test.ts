...
const NOT_TEXT: [string, string][] = [
  ...
  ["users", "workspace"],
  /*
   * Статистика: строки-критерии пресета и колонки модели — ссылки на
   * методику, версию, полосы и варианты плюс числа и коды; подписи к ним
   * экран берёт из содержимого версии на языке читателя.
   */
  ["filter_presets", "criteria"],
  ["stat_models", "columns"],
  /*
  ...
  /*
   * Техпанель, сигналы (миграция 0095): исходы отправки по каналам, список
   * каналов правила и кадры стека ошибки клиента — коды и пути, а не текст
   * для людей; слова к ним экран берёт из словаря.
   */
