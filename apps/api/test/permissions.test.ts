...
describe("техпанель не входит в клинический набор", () => {
  /*
   * Набор психолога собран вычитанием из всех прав, и каждое новое право
   * попадает в него само. Права техпанели попали бы так каждому клиническому
   * администратору: ops.read — логи и состояние базы, ops.manage — режим
   * обслуживания и флаги для всех. Оба выдаются только личным исключением.
   */
  test("ни ops.read, ни ops.manage", () => {
    expect(PSYCHOLOGIST_PERMISSIONS).not.toContain("ops.read");
    expect(PSYCHOLOGIST_PERMISSIONS).not.toContain("ops.manage");
  });
});
