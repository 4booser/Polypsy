...
describe("настройки рабочего места", () => {
  ...
  test("сигнальный раздел рельсы не спрятать запросом мимо экрана", async () => {
    /*
     * Экран настроек такой пункт не даёт отключить — галочка стоит намертво.
     * Но экран это только кнопка: тот же запрос отправляется руками, и если
     * сервер его примет, человек останется без единственного места, где видно
     * неразобранные случаи риска. Правило живёт в схеме, значит проверяется
     * на любом пути, а не на том, по которому ходит наша кнопка.
     */
    const res = await api("/api/auth/me/workspace", adminA.token, {
      method: "PUT",
      body: JSON.stringify({ railHidden: ["nav.cases"] }),
    });
    expect(res.status, "сервер принял скрытие случаев риска").toBe(400);

    // а обычный раздел прячется без возражений
    const ok = await api("/api/auth/me/workspace", adminA.token, {
      method: "PUT",
      body: JSON.stringify({ railHidden: ["nav.surveys"] }),
    });
    expect(ok.status, "обычный раздел скрыть не дали — настройка бесполезна").toBe(200);
  });
...
describe("установщик заводит пригодную учётную запись", () => {
  test("адрес по умолчанию проходит проверку входа", async () => {
    /*
     * По умолчанию стояло «admin@local», и войти с ним нельзя: домен без
     * точки zod за адрес не считает. Установщик заводил единственную
     * учётную запись свежего экземпляра и делал её непригодной — молча, с
     * бодрым «создан первый суперадминистратор» и напечатанным один раз
     * паролем. Обнаруживалось это в первую же попытку войти, когда
     * поправить было уже нечем.
     *
     * Адрес читается из исходника установщика, а не повторяется здесь:
     * иначе проверка сверяет константу сама с собой и переживёт любую
     * правку умолчания.
     */
    const src = readFileSync(resolve(import.meta.dir, "../src/install.ts"), "utf8");
    const found = /ADMIN_EMAIL \?\? "([^"]+)"/.exec(src);
    expect(found, "не нашёл адрес по умолчанию в install.ts").toBeTruthy();

    const { loginSchema } = await import("@quizzy/shared");
    const parsed = loginSchema.safeParse({ email: found![1], password: "какой-нибудь" });
    expect(parsed.success, `адрес «${found![1]}» не проходит проверку входа`).toBe(true);
  });
});
