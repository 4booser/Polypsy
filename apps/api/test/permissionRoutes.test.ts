...
/**
 * Подсказки меню живут ровно в одном месте — в /me.
 *
 * Консоль по ним решает, показывать ли вкладку «Приглашения» и пункт
 * «Права». Ограничением они не являются: правила проверяются на самих
 * маршрутах. Но пока подсказка существует в одном месте, показанное меню и
 * разрешённое действие не могут разойтись; как только она появится ещё и в
 * ответе на вход, расхождение станет вопросом времени — и заметят его так
 * же, как в прошлый раз: человек войдёт и не увидит того, на что имеет
 * право.
 *
 * Отсюда две проверки-близнеца: /me подсказки отдаёт, вход — нет. Вторая
 * важнее первой, и она же неочевиднее: «дослать поле в ответ на вход»
 * выглядит как починка, а на деле заводит второй источник истины.
 */
describe("подсказки меню приходят только из /me", () => {
  test("/me отдаёт ступень должностей и право на приглашения", async () => {
    const res = await api("/api/auth/me", adminA.token);
    expect(res.status).toBe(200);
    for (const hint of ["ladderRank", "canInvite"]) {
      expect(
        hint in res.body,
        `/me не отдаёт «${hint}» — консоль спрячет то, на что у человека есть право`,
      ).toBe(true);
    }
  });

  test("ответ на вход подсказок не несёт", async () => {
    /*
     * Своя учётка, а не общая: у заведённых посевом адреса без домена
     * («a@test»), и вход их отклоняет проверкой формата ещё до пароля.
     * Тест на этом и споткнулся — отказ был настоящим, но не про то.
     */
    const email = `login-hints-${crypto.randomUUID().slice(0, 8)}@test.dev`;
    await makeUser("admin", email);
    const res = await api("/api/auth/login", "", {
      method: "POST",
      body: JSON.stringify({ email, password: "secret12345" }),
    });
    expect(res.status, "подопытная учётка не вошла — проверять нечего").toBe(200);

    const leaked = ["ladderRank", "canInvite"].filter((hint) => hint in (res.body.user ?? {}));
    expect(
      leaked,
      "подсказки меню продублированы в ответе на вход: два источника истины разойдутся, и молча",
    ).toEqual([]);
  });
});
