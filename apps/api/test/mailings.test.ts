...
describe("отправка", () => {
   ...
   *
   * Защита двойная — ранняя проверка статуса и условие `status = 'draft'` в
   * самом UPDATE, — и каждая половина в одиночку держит удар. Мутация:
   * снять обе из PATCH — проверка падает и называет рассылку, у которой
   * тема сменилась после отправки; снять обе из send — падает та же
   * проверка на повторной отправке (получатели получили бы дубль).
   */
  ...
  /**
   * Отправленную не удалить — только скрыть. Защита двойная — ранняя
   * проверка статуса и условие `status = 'draft'` в самом DELETE, — и
   * каждая половина в одиночку держит удар. Мутация: снять обе — проверка
   * падает и называет рассылку, исчезнувшую вместе с ответами получателей.
   */
...
describe("получатель", () => {
  ...
  /**
   * Ответ — один раз. Защита двойная — проверка до записи и условие
   * `answer is null` в самом UPDATE (второе закрывает два нажатия подряд), —
   * и каждая половина в одиночку держит удар. Мутация: снять обе — проверка
   * падает и называет получателя, чей ответ переписался.
   */
