...
/*
 * Про гонку захвата записи проверки нет намеренно.
 *
 * Захват теперь одним оператором с `for update skip locked` — двое одну
 * запись не возьмут по построению. Но воспроизвести гонку в этом стенде не
 * выходит: параллельные вызовы через Promise.all драйвер успевает
 * сериализовать, и проверка проходит одинаково на исправном и на сломанном
 * коде. Такая проверка не защищает, а создаёт впечатление защиты — и первая
 * её редакция именно этим и была.
 */
