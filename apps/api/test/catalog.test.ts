...
describe("содержимое каталога", () => {
  ...
  test("каждый пункт входит хотя бы в одну шкалу", () => {
     ...
     * суммируют все свои пункты.
     *
     * Исключение одно — отсеивающий вопрос (gate), от ответа на который
     * зависит, покажут ли остальные (PC-PTSD-5: «If no, screen total = 0»).
     * В сумму он не входит по бланку, но и не заполняется впустую. Узнаётся
     * он по устройству, а не по списку имён: на него ссылается условная
     * логика других пунктов. Отвергнутая альтернатива — завести ему шкалу
     * ради этой проверки: у бланка подшкал нет.
     */
    for (const entry of CATALOG) {
