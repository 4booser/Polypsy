...
describe("содержимое каталога", () => {
  ...
  test("каждый пункт входит хотя бы в одну шкалу", () => {
     ...
     * двух случаев, и оба узнаются не по списку имён.
     *
     * Отсеивающий вопрос (gate), от ответа на который зависит, покажут ли
     * остальные (PC-PTSD-5: «If no, screen total = 0»). В сумму он не входит
     * по бланку, но и не заполняется впустую; узнаётся по устройству — на него
     * ссылается условная логика других пунктов. Отвергнутая альтернатива —
     * завести ему шкалу ради этой проверки: у бланка подшкал нет.
     *
     * Пункты, которые сама методика запрещает считать (фильтры и отдельный
    ...
    for (const entry of CATALOG) {
      ...
      const gates = new Set(
        (entry.draft.questions ?? []).flatMap((q) => (q.logic ?? []).map((r) => r.sourceIndex + 1)),
      );
      const excused = new Set(entry.notScored?.items ?? []);
      const total = entry.draft.questions?.length ?? 0;
      for (let i = 1; i <= total; i++) {
        if (gates.has(i) || excused.has(i)) continue;
