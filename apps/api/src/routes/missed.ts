  ...
  const groups = [
    ...
    },
    {
      ...
      items: runs.map((r) => ({
        ...
        severity: null,
        /*
         * На батареи, а не на «/schedules»: такого экрана в консоли нет.
         *
         * Ссылка собиралась сервером, вела в никуда и молча роняла человека
         * на сводку — маршрутизатор отправляет неизвестный путь на «/», и
         * выглядело это как «не открылось, само куда-то ушло». Экран
         * расписаний убрали, а строка осталась.
         *
         * Расписание назначает батарею; её назначения и видны на /batteries —
         * то есть ровно то, что прогон сделал. Хронология ведёт туда же по
         * той же причине (timeline.ts, вид «assignment»).
         */
        href: `/batteries`,
      })),
    },
