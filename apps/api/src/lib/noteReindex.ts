...
import { indexOf } from "./searchIndex";
import { log } from "./log";
 ...
 *
 * Не при старте приложения: на десятках тысяч записей это минуты работы и
 * расшифровка всего массива, и делать такое неявно при каждом перезапуске
 * нельзя. Запускается командой (apps/api/src/reindex.ts) или кнопкой
 * «Запустити зараз» техпанели (lib/opsManual.ts) — одна функция на оба пути,
 * иначе две копии разошлись бы на первой же правке индекса.
 *
 * Идёт внутри того контекста, что открыл вызывающий (системного): политики
 * строк закрывают записи всем, кроме их авторов.
 */
export async function reindexNotes(): Promise<{ indexed: number; skipped: number }> {
  const rows = await db.select().from(patientNotes);
