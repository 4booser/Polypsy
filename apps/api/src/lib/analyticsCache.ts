...
/**
 * Сбросить кэш процесса — «пересчёт аналитики» из техпанели.
 *
 * Аналитика в системе не хранится посчитанной: она считается на лету и
 * кэшируется здесь по отпечатку данных. Поэтому «пересчитать» — значит
 * забыть кэш, и следующее открытие каждого экрана посчитает заново. Нужен
 * этот сброс, когда данные менялись мимо сдачи (правка норм, ручная
 * правка в базе, восстановление из бэкапа): отпечаток по числу сдач и
 * последней сдаче таких правок не видит. Возвращает, сколько записей было.
 */
export function clearAnalyticsCache(): number {
  const n = cache.size;
  cache.clear();
  return n;
}
