 ...
 */
/*
 * Прежняя редакция утверждала «пороги встановлені для оригіналу» и велела
 * «звірити з офіційним україномовним варіантом» — и приклеивалась к методикам
 * без порогов (PSS-10, RSES, Brief COPE) и без официального перевода
 * (большинство каталога). Описание тогда противоречило само себе строкой
 * раньше. Условная форма верна для любой методики; отвергнута вторая
 * функция «describe без порогов» — выбор между ними снова делал бы автор
 * методики, и ошибиться можно было бы тем же способом.
 */
export const WORKING_TRANSLATION = L(
  "Робочий переклад. Якщо в методики є пороги, їх встановлено для оригіналу; коли з’явиться офіційний україномовний варіант, звірити з ним.",
  "Рабочий перевод. Если у методики есть пороги, они установлены для оригинала; когда появится официальный украиноязычный вариант, сверить с ним.",
);
