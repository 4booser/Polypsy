...
export const requireAuth = createMiddleware<AppEnv>(async (c, next) => {
  ...
  /*
   * Выключенная учётка останавливается здесь, и раньше проверки отзыва.
   *
   * Выключение и так сдвигает границу токенов, но тогда живой токен получил
   * бы «сессия истекла», клиент пошёл бы менять refresh, и человек читал бы
   * про истёкшую сессию вместо того, что случилось на самом деле. Проверка
   * стоит ничего: строка пользователя уже прочитана. И она не зависит от
   * того, успел ли сдвиг границы, — это второй замок, а не первый.
   */
