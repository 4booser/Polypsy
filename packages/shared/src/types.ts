...
export interface User {
  ...
  rank: string | null;
  /** Населённый пункт — фильтр выборок в «Статистиці»; открытым текстом, см. миграцию 0082 */
...
export interface StatRunScale {
   ...
   *
   * Полосы шкалы и остаток — одна группа: под порогом хоть одна ячейка —
   * скрыта вся группа, иначе скрытая называется вычитанием из основания.
   */
  rest: StatCell;
}
...
export interface StatRunQuestion {
   ...
   * ответивших. Варианты вопроса и остаток — одна группа: под порогом хоть
   * одна ячейка — скрыта вся группа. У вопроса с несколькими вариантами
   * остаток — дополнение объединения, и порог к каждой ячейке применяется
   * с обоих краёв.
   */
  rest: StatCell;
}
 ...
 * «overlap» — она отличается от соседней колонки той же модели горсткой
 * людей, и вычитание колонок описывает эту горстку по всем показателям
 * сразу. Экран в обоих случаях рисует строки с подписью «менше 5»: причина
 * нужна не подписи, а разбору — по ней видно, почему колонка над порогом
 * всё-таки закрыта.
 */
export type StatSuppressReason = "small" | "overlap";
...
export interface StatRunColumn {
   ...
   * все ячейки под ним; то же с колонкой, закрытой разностью составов.
   */
   ...
   * колонке ничего не помечено. Подавляется с обоих краёв: «четверо из пяти»
   * называет пятого так же точно, как «один из пяти». Скрывается и когда
   * скрыта хоть одна группа с помеченной ячейкой (сумма поверх скрытого
   * слагаемого называет его вычитанием) и когда помеченные множества
   * пересекаются меньше чем на порог: показанные ячейки и объединение
   * поверх них называют пересечение.
   */
