...
export const PERMISSION_GROUPS = [
  ...
  },
  {
    ...
    permissions: [
      "analytics.read",
      /*
       * Статистика — отдельное право, а не часть analytics.read. Аналитика
       * смотрит на методику целиком; статистика режет людей произвольными
       * фильтрами по полу, возрасту и населённому пункту — квази-
       * идентификаторам, которые в сумме сужают круг до одного. Порог
       * малых ячеек это закрывает, но право решать, кому вообще давать
       * такую машину, должно быть у учреждения, а не следовать за
       * аналитикой автоматически. На макете это и отдельный пункт меню.
       */
      "statistics.read",
      "cohorts.read",
      "export.deidentified",
  ...
  /*
   * Техническая служба — для разработчиков и того, кто держит сервер.
   *
   * Отдельной группой, а не ещё одним правом «Адміністрування»: смотреть,
   * как работает система (логи, ошибки, нагрузка, база, фоновые задачи), и
   * вести людей — разные работы, и выдавать их хочется разным людям.
   * Разработчику нужна панель, но не нужны пациенты: ops.read не открывает
   * ни одной клинической записи, а управление учётками и журнал доступа
   * остаются за своими правами (users.manage, audit.read).
   *
   * Выдаётся человеку личным исключением через общий выбор права на экране
   * прав: разработчик — конкретная учётка, и роль под одного заводить незачем.
   * В быстрые исключения не внесено: там ровно три случая, названные на
   * экране словами (permissions.test.ts).
   */
  {
    code: "ops",
     ...
     * ручной запуск фоновых задач — то, от чего меняется работа у всех.
     * Самое опасное (SQL-консоль, вход под другим человеком, ротация
     * ключей) не выдаётся никаким правом: только суперадмину.
     */
 ...
 * разработчику личным исключением (группа «Технічна служба»).
 */
/*
 * Техпанель (ops.read, ops.manage) в набор тоже не входит. Набор собран
 * вычитанием из всего справочника, и новое право попадает в него само, если
 * его не назвать здесь: так ops.read молча досталось бы каждому
 * администратору группы — а техпанель выдаётся разработчику личным
 * исключением (группа «Технічна служба»), а не всем, кто ведёт пациентов.
 */
/*
...
/*
 * Пункты рельсы, которые человек не может убрать с глаз.
 *
 * Скрывать пункты можно и нужно: рельса на тринадцать разделов стоит
 * внимания при каждом открытии консоли, а половиной из них конкретный
 * специалист не пользуется. Но три из них носят не раздел, а СИГНАЛ: рядом с
 * ними стоит число — сколько случаев риска не разобрано, сколько задач в
 * очереди, сколько человек ждёт сегодня. Спрятав такой пункт, человек прячет
 * не меню, а знание о том, что его ждёт работа; в психологическом отделении
 * это ровно то, ради чего всё и написано.
 *
 * Поэтому список живёт здесь, рядом с правами, а не в настройках внешнего
 * вида: это правило безопасности, а не вкус. На экране настроек такие пункты
 * показаны с объяснением, а не просто без галочки, — иначе человек решит,
 * что это поломка, и пойдёт искать обход.
 */
export const ALWAYS_VISIBLE_RAIL = ["nav.dashboard", "nav.worklist", "nav.cases"] as const;
