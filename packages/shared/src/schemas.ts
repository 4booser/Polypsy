...
/**
 * Динамика одного человека: ?survey=<id> — только по одной методике.
 *
 * Графикам прохождения нужна динамика ОДНОЙ методики, а маршрут без
 * параметра считает альфу для RCI по каждой методике человека, поднимая на
 * каждую до трёхсот прохождений с ответами. У пациента с десятью
 * методиками это десять таких выборок ради одной.
 */
export const respondentDynamicsQuery = z.object({
  survey: z.string().min(1).max(64).optional(),
});
