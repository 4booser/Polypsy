...
Полнота покрытия проверяется тестом. Политики писались один раз, а таблицы
добавлялись позже — так четыре таблицы с клиническими данными (случаи риска,
направления, заключения, назначения) и оказались вне сетки. Дырявый пояс
обесценивает себя целиком, а помнить про RLS при каждой новой таблице никто
не обязан, поэтому за этим следит тест, а не человек.
...
- **Защита от вредоносного суперадмина.** Он может выдать себе доступ к любой
  ...
  запись, но и не помешает действию. SQL-консоль техпанели этого не меняет:
  она только читает (транзакция READ ONLY с откатом, один оператор,
  своё соединение на запрос), видит то же, что суперадмин через политики
  строк, шифрованные поля отдаёт шифртекстом, и каждый её запрос — с
  причиной и текстом — ложится в журнал.
